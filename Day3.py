...
lists = """LLBPGtltrGPBMMsLcLMMVMpVRhhfCDTwRwRdTfwDllRRRDhC
...
for l in lists:
	length = len(l)
	firstL = set(l[:length//2])
	secondL = set(l[length//2:])
	
	letter = firstL.intersection(secondL).pop()
...
# Second case
size = 3
splited = [lists[x:x+size] for x in range(0, len(lists), size)]

total = 0
for s in splited:
	set1, set2, set3 = set(s[0]), set(s[1]), set(s[2])
	
	letter = set1.intersection(set2, set3).pop()
	total += mapping[letter]
